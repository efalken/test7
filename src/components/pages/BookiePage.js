...
class BookiePagejs extends Component {
  ...
  wdTaker() {
    const stackId = this.contracts["BetSwap"].methods.withdrawEth.cacheSend({
  ...
  inactivateBook() {
    const stackId = this.contracts["BetSwap"].methods.inactiveBook.cacheSend();
  }
  ...
  render() {
   ...
   let unusedCapital = "0";
    ...
    if (this.weekKey in this.props.contracts["BetSwap"].betEpoch) {
      week = this.props.contracts["BetSwap"].betEpoch[this.weekKey].value
    }
   ...
   scheduleString = this.props.contracts["BetSwap"].showSchedString[this.scheduleStringKey].value;
    ...
    return (
      <div>
        ...
        <Flex width="100%"
        justifyContent="marginLeft" >
        <Text size="14px" weight="300"> This page helps LPs understand their netLiab
