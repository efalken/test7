...
contract Oracle {
    ...
    // betAmount*propOdds/1000 is the net payoff for a winning bet
    ...
    event Proposal(string process, uint256 timestamp, address sender);
    ...
    function pullEther() external {
        uint ethTrans = bettingContract.oracleBalance();
        bettingContract.withdrawOracle();
        ethBalance += ethTrans;
    }
    ...
    function dividendClaim() external {
        ...
        uint8 betEpoch2 = bettingContract.betEpoch();
        require(betEpoch2 % 3 == 0);
        require(tokenBalances[msg.sender] > 0);
        ...
        uint256 ethTrans = (sharesToSell * ethBalance) / localSupply;
        ethBalance -= ethTrans;
        payable(msg.sender).transfer(ethTrans);
        localSupply -= sharesToSell;
        emit Funding(0, sharesToSell, ethTrans, betEpoch2, block.timestamp, msg.sender);
    }
    ...
    function initPost(
        ...
        require(underReview == 0, "Under Review");
        underReview = 1;
        post();
    ...
    function settleProcess() external {
        ...
        reset();
        delete propResults;
        if (betEpoch % 3 == 0) timerEthDiv = block.timestamp + 1 days;
    }

    function paramProcess() external {
        require(underReview == 4);
        // needs at least 3 hours or a clear majority decision
        require (block.timestamp > timer);
        if (voteYes > voteNo) {
            tokenBalances[proposer] += bond;
    ...
    function withdrawTokens(uint256 amt) external {
        ...
        emit Funding(0, amt, 0, betEpoch, block.timestamp, msg.sender);
    }
    ...
    function depositTokens(uint256 amt) external {
        ...
        emit Funding(amt, 0, 0, betEpoch, block.timestamp, msg.sender);
    }
