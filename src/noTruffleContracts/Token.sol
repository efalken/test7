...
contract Token {
    ...
    // this variable keeps track of proposals so that token holders cannot vote twice
    uint32 public votePropNumber;
    // this allows users to know which contracts these token holders currently support
    address public bettingContract;
    // token holders can only vote on proposals from the approved oracle contract
    ...
    function processVote() external {
        // this gives the vote at least 7 days from its initial proposal for voting
        ...
        // all approved contracts will be registered in the token contracts whiteList mapping
            ...
            _balances[proposedAddresses[2]] += 1e5 ether;
        }
        emit VoteOutcome(
            proposedAddresses[0],
            proposedAddresses[1],
        ...
        // regardless of outcome this increment allows those who recently voted to transfer their tokens again
        votePropNumber++;
    }
        ...
        external
    {
        ...
        require(_balances[msg.sender] >= 1e5 ether);
        ...
        // vote is processed in 7 days
        // change to days
        timer = block.timestamp + 7 seconds;
        ...
        _balances[msg.sender] -= 1e5 ether;
